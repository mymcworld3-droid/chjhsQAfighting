...
test('admin recipe editor mirrors the eight-slot refinery limit in the UI', () => {
  ...
  assert.match(enhancement, /已使用|煉器陣材料格/);
  assert.match(enhancement, /total > SLOT_LIMIT/);
  assert.match(enhancement, /save\.disabled = invalid/);
  assert.match(enhancement, /最多 8 個/);
  assert.match(enhancement, /不看 8 格中的排列順序/);
});

test('catalog enforces eight total materials per artifact recipe', () => {
  assert.match(catalog, /totalMaterials > MAX_ARTIFACT_RECIPE_MATERIALS/);
