...
(function () {
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      ...
      .aam-list{display:grid;gap:7px}.aam-item{display:grid;grid-template-columns:40px minmax(0,1fr) auto;gap:9px;align-items:center;padding:9px;border:1px solid rgba(255,255,255,.07);border-radius:13px;background:rgba(255,255,255,.018)}.aam-icon{width:38px;height:38px;display:grid;place-items:center;overflow:hidden;border-radius:11px;border:1px solid rgba(216,177,93,.25);color:#f0d17a;background:#171005;font-weight:900}.aam-icon img{width:100%;height:100%;display:block;object-fit:cover}.aam-copy{min-width:0}.aam-copy strong{color:#eee1c7;font-size:10px}.aam-meta{margin-top:3px;color:#887a63;font-size:7px}.aam-effects{display:flex;gap:4px;flex-wrap:wrap;margin-top:5px}.aam-effects span{padding:2px 5px;border-radius:999px;background:rgba(216,177,93,.05);color:#baa77d;font-size:6px}.aam-item-actions{display:grid;gap:5px;min-width:78px}.aam-edit,.aam-approve{min-height:32px;padding:0 10px;border-radius:9px;font-size:8px;font-weight:900}.aam-edit{border:1px solid rgba(216,177,93,.22);background:rgba(216,177,93,.05);color:#dbc078}.aam-approve{border:1px solid rgba(74,222,128,.28);background:rgba(22,101,52,.12);color:#86efac}.aam-pending{display:inline-flex;align-items:center;gap:4px;margin-left:6px;padding:2px 6px;border:1px solid rgba(251,191,36,.38);border-radius:999px;background:rgba(146,64,14,.16);color:#fbbf24;font-size:6px;font-weight:900}.aam-item.is-pending{border-color:rgba(251,191,36,.46)!important;box-shadow:inset 0 0 24px rgba(251,191,36,.045),0 0 20px rgba(251,191,36,.04)}.aam-ai-meta{margin-top:4px;color:#b99753;font-size:6px}
      ...
      .aam-effect-hint{margin-top:6px;padding:6px 8px;border-left:2px solid rgba(216,177,93,.4);border-radius:5px;background:rgba(216,177,93,.04);color:#c3ae7f;font-size:8px;line-height:1.55;overflow-wrap:anywhere}.aam-effect-hint.is-outside{border-color:#e3a45e;color:#e3be82}
      .aam-recipe-owner-editor,.aam-recipe-listing{box-sizing:border-box;min-width:0;margin-top:12px;padding:11px;border:1px solid rgba(216,177,93,.2);border-radius:12px;background:rgba(216,177,93,.025)}
      .aam-recipe-owner-editor h4,.aam-recipe-listing h4{margin:0 0 8px;color:#f2d793;font-size:10px;font-weight:900}
      .aam-owner-current-uid{overflow-wrap:anywhere;word-break:break-word}.aam-owner-toggle{display:flex;align-items:center;gap:8px;margin:10px 0;color:#ead6aa!important;font-size:9px;line-height:1.5;cursor:pointer}.aam-owner-toggle input[type="checkbox"]{box-sizing:border-box;display:block;flex:0 0 16px;width:16px!important;height:16px!important;min-height:0!important;margin:0;padding:0;accent-color:#d8b15d}.aam-owner-toggle span{min-width:0}
      .aam-owner-fields{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;min-width:0}
      .aam-owner-fields label{display:grid;gap:5px;min-width:0;overflow-wrap:anywhere;color:#cdb98e;font-size:9px}
      .aam-owner-fields input,.aam-admin-sale-row input{box-sizing:border-box;width:100%;min-width:0;min-height:36px;padding:8px;border:1px solid rgba(216,177,93,.25);border-radius:8px;background:#100e0a;color:#f4e3bb;font-size:10px}
      .aam-admin-sale-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;align-items:center}
      ...
      @media(max-width:540px){.aam-owner-fields,.aam-admin-sale-row{grid-template-columns:minmax(0,1fr)}.aam-admin-list-recipe{min-height:36px}}
  ...
  function currentEditorStage(modal) {
    let stage = 1;
    modal.querySelectorAll('[data-recipe-artifact]').forEach((input) => {
      if (Number(input.value) <= 0) return;
      const artifactId = input.dataset.recipeArtifact;
      if (ARTIFACT_CATALOG.some((item) => item.id === artifactId)) {
        stage = Math.max(stage, Math.min(3, artifactRecipeDepth(artifactId) + 2));
      }
    });
    return stage;
  }
  ...
  function depthHintForRealm(bounds, realmIndex, inverse = false, integer = false) {
    ...
    const min = Number(bounds.min) + step * start;
    const max = Number(bounds.min) + step * (start + 3);
    if (integer) {
      const roundedMin = Math.ceil(min - 1e-9);
      return [roundedMin, Math.max(roundedMin, Math.floor(max + 1e-9))];
    }
    return [Number(min.toFixed(4)), Number(max.toFixed(4))];
  }

  function loadDepthHints(stage) {
    if (depthHintCache.has(stage)) return depthHintCache.get(stage);
    const request = fetch('/api/artifact-depth-effect-ranges?stage=' + stage)
      .then((response) => {
        if (!response.ok) throw new Error('HTTP ' + response.status);
        return response.json();
      })
      .then((payload) => {
        if (!Array.isArray(payload.ranges)) throw new Error('數值表格式錯誤');
        return new Map(payload.ranges.map((range) => [range.type, range]));
      })
      .catch((error) => {
        console.warn('[Admin artifact range hints]', error);
        return null;
      });
    depthHintCache.set(stage, request);
    return request;
  }

  function showEffectHint(row, modal, ranges) {
    const display = row.querySelector('[data-aam-effect-hint]');
    if (!display) return;
    const type = row.querySelector('[data-aam-effect-type]')?.value || '';
    const realm = modal.querySelector('#aam-realm')?.value || '煉氣';
    const index = ARTIFACT_REALMS.findIndex((entry) => entry.name === realm);
    const stage = currentEditorStage(modal);
    const prefix = '參考範圍 · ' + realm + '／第 ' + stage + ' 煉：';
    if (index < 1) {
      display.textContent = prefix + '不生成凡人法寶；既有法寶仍可手動編輯。';
      return;
    }
    if (!ranges) {
      display.textContent = '參考範圍暫時無法載入；不影響手動編輯與儲存。';
      return;
    }
    if (index < (effectMinRealms[type] || 0) || !ranges.has(type)) {
      display.textContent = prefix + '此境界／深度未列入 AI 自動生成的功能；仍可手動設定。';
      return;
    }
    const base = ranges.get(type);
    if (!['value','multiplier'].includes(base.field)) {
      display.textContent = prefix + (base.note || '固定規則，不需輸入數值');
      return;
    }
    const setting = window.getArtifactEffectBounds?.()?.[String(stage)]?.[type];
    ...
    if (base.field === 'multiplier') {
      ...
      text += '；持續 ' + number(durationMin) + '～' + number(durationMax) + ' 分鐘';
    }
    const input = row.querySelector(base.field === 'value' ? '[data-aam-effect-value]' : '[data-aam-effect-multiplier]');
    const current = Number(input?.value);
    const outside = input?.value?.trim() && Number.isFinite(current) && (current < min - 1e-8 || current > max + 1e-8);
    if (outside) text += '；目前填值超出參考範圍';
    display.textContent = text + '（僅供參考，不限制手動填寫）';
    display.classList.toggle('is-outside', !!outside);
  }

  function refreshEffectHints(modal) {
    if (!modal?.isConnected) return;
    const stage = currentEditorStage(modal);
    const apply = (ranges) => {
      if (!modal.isConnected || currentEditorStage(modal) !== stage) return;
      modal.querySelectorAll('.aam-effect-row').forEach((row) => showEffectHint(row, modal, ranges));
    };
    modal.querySelectorAll('[data-aam-effect-hint]').forEach((el) => { el.textContent = '正在讀取效果參考上下限…'; });
    void loadDepthHints(stage).then(apply);
  }
  ...
  function effectRow(effect = {}) {
    ...
    const sync = () => {
      ...
      if (row.isConnected) refreshEffectHints(row.closest('.aam-modal'));
    };
    row.querySelector('[data-aam-effect-type]').addEventListener('change', sync);
    row.addEventListener('input', () => { if (row.isConnected) refreshEffectHints(row.closest('.aam-modal')); });
  ...
  function openEditor(item = null) {
    ...
    modal.innerHTML = `<section class="aam-card" role="dialog" aria-modal="true"><h3>${editing ? '編輯法寶' : '新增法寶'}</h3><p class="aam-note">這裡直接維護全站正式法寶清單。建立後 ID 會鎖定，避免玩家既有背包與裝備失聯。右側「可用功能」可直接加入效果。</p><div class="aam-editor-layout"><div class="aam-editor-main"><div class="aam-grid"><div class="aam-field"><label>法寶 ID（英文小寫與 -）</label><input id="aam-id" maxlength="64" ${editing ? 'readonly' : ''} value="${escapeHtml(item?.id || '')}" placeholder="例如 thunder-seal"></div><div class="aam-field"><label>名稱</label><input id="aam-name" maxlength="80" value="${escapeHtml(item?.name || '')}"></div><div class="aam-field"><label>圖示（1–4 字）</label><input id="aam-icon" maxlength="4" value="${escapeHtml(item?.icon || '◆')}"></div><div class="aam-field"><label>境界</label><select id="aam-realm">${ARTIFACT_REALMS.filter((realm) => realm.name !== '凡人' || item?.realm === '凡人').map((realm) => `<option value="${realm.name}" ${realm.name === (item?.realm || '築基') ? 'selected' : ''}>${realm.name}</option>`).join('')}</select></div><div class="aam-field"><label>分類</label><select id="aam-category">${categoryOptions(item?.category)}</select></div><div class="aam-field"><label>正式器型</label><select id="aam-weapon-form"><option value="">未指定</option>${ARTIFACT_WEAPON_FORMS.map((form) => `<option value="${escapeHtml(form)}" ${form === (item?.weaponForm || '') ? 'selected' : ''}>${escapeHtml(form)}</option>`).join('')}</select></div><div class="aam-field"><label>煉器手法</label><select id="aam-forge-method">${ARTIFACT_FORGE_METHODS.map((method) => `<option value="${escapeHtml(method)}" ${method === (item?.forgeMethod || '自由發揮') ? 'selected' : ''}>${escapeHtml(method)}</option>`).join('')}</select></div><div class="aam-field"><label>裝備欄位（裝備效果才需要）</label><select id="aam-slot"><option value="">不使用裝備欄位</option>${ARTIFACT_EQUIP_SLOTS.map((slot) => `<option value="${escapeHtml(slot)}" ${slot === (item?.equipSlot || '') ? 'selected' : ''}>${escapeHtml(slot)}</option>`).join('')}</select></div><div class="aam-field"><label>打造金幣</label><input id="aam-gold" type="number" min="0" step="1" value="${Number(item?.craft?.gold) || 0}"></div><div class="aam-field"><label>每次打造數量</label><input id="aam-yield" type="number" min="1" step="1" value="${Math.max(1, Number(item?.craft?.yield) || 1)}"></div><div class="aam-field full"><label>說明</label><textarea id="aam-description" maxlength="500">${escapeHtml(item?.description || '')}</textarea></div></div><div class="aam-effects-editor"><div class="aam-effects-head"><span>法寶效果</span><button type="button" id="aam-add-effect" class="aam-add-effect"><i class="fa-solid fa-plus"></i> 新增效果</button></div><div id="aam-effect-list"></div></div><section class="aam-recipe-owner-editor"><h4><i class="fa-solid fa-crown"></i> 配方擁有人（管理員）</h4><p class="aam-note">現任：${escapeHtml(item?.recipeOwnerName || '公共配方／無首發者')}<br>UID：<span class="aam-owner-current-uid">${escapeHtml(item?.recipeOwnerUid || '無')}</span><br>更改後不轉移已購買者的製作指南；原擁有人的待售委託將失效。</p><label class="aam-owner-toggle" for="aam-owner-change"><input type="checkbox" id="aam-owner-change" ${editing ? '' : 'disabled'}><span>我確認要更改此配方的擁有人</span></label><div class="aam-owner-fields"><label for="aam-owner-uid">新擁有人 UID（留空設為公共配方）<input id="aam-owner-uid" maxlength="128" value="${escapeHtml(item?.recipeOwnerUid || '')}" placeholder="玩家 Firebase UID"></label><label for="aam-owner-name">新擁有人名稱<input id="aam-owner-name" maxlength="36" value="${escapeHtml(item?.recipeOwnerName || '')}" placeholder="修士名稱"></label></div></section>${editing && getArtifactRecipe(item.id).length ? `<section class="aam-recipe-listing"><h4><i class="fa-solid fa-store"></i> 管理員配方上架</h4><p class="aam-note">由管理員作為賣家上架永久製作指南；不改變配方擁有權。請先儲存配方或擁有人變更，再上架。</p><div class="aam-admin-sale-row"><input type="number" id="aam-listing-price" min="1" max="1000000000" step="1" value="500" aria-label="配方售價（靈石）"><button type="button" class="aam-admin-list-recipe" data-aam-list-recipe="${escapeHtml(item.id)}"><i class="fa-solid fa-tags"></i> 上架配方</button></div></section>` : ''}${recipeEditorMarkup(item)}<div id="aam-status" class="aam-note" style="margin-top:10px"></div></div><aside class="aam-guide"><h4><i class="fa-solid fa-list-check"></i> 可用功能</h4><p>依類別展開或縮小；功能區可獨立捲動，點選功能即可加入左側法寶效果。</p>${effectGuideMarkup()}<div class="aam-guide-note"><b>裝備類：</b>必須從四個正式裝備欄位中選擇。<br><b>連擊：</b>不論組合多少效果，總機率硬上限 10%。<br><b>鏡映：</b>每場固定複製敵方一項可複製戰鬥效果。<br><b>限時類：</b>催動時消耗 1 件法寶。<br><b>排除錯項：</b>可指定問道、鬥法、洞天。</div></aside></div><div class="aam-actions"><button type="button" class="aam-cancel">取消</button><button type="button" class="aam-save">${editing ? '儲存變更' : '建立法寶'}</button></div></section>`;
    ...
    modal.querySelector('#aam-add-effect').onclick = () => {
      effectList.appendChild(effectRow(defaultEffect('equip_attack_flat')));
      refreshEffectHints(modal);
    };
    ...
    modal.querySelector('#aam-realm').addEventListener('change', () => refreshEffectHints(modal));
    refreshEffectHints(modal);
  ...
  async function saveFromModal(modal, originalId) {
    ...
    const raw = {
      ...
      weaponForm: modal.querySelector('#aam-weapon-form').value,
      forgeMethod: modal.querySelector('#aam-forge-method').value,
      coreEffect: original?.coreEffect || '',
  ...
  function boot() {
    ...
    window.addEventListener('artifact-effect-bounds-updated', () => refreshEffectHints(document.getElementById(MODAL_ID)));
