...
(function () {
  ...
  function card(listing, own = false) {
    ...
    return `<article class="pm-card pm-${esc(listing.type)}"><div class="pm-card-head"><span class="pm-icon">${esc(listing.itemIcon)}</span><div><b>${esc(listing.itemName)}</b><small>${esc(TYPE_LABEL[listing.type] || '商品')} · ${esc(listing.itemRealm)} · ×${qty(listing.quantity)}</small></div></div>
  ...
  function installStyle() {
    ...
    style.textContent = `
      ...
      #player-market .pm-material .pm-icon{border-radius:50%;background:radial-gradient(circle at 30% 20%,#9e8246,#2d220f 60%,#120e09);box-shadow:inset 0 0 0 2px rgba(255,235,174,.09)}
      #player-market .pm-artifact .pm-icon,#player-market .pm-recipe .pm-icon{border-radius:10px;box-shadow:inset 0 0 0 2px rgba(255,235,174,.1),0 0 14px rgba(230,189,103,.1)}
