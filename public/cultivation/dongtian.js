...
(function () {
  ...
  function firstCompletionCultivation(correctCount) {
    ...
    return correct; // 首次通關每答對一題，增加一修為；零答對不加修為。
  }
  ...
  const css = `
    ...
    @media(max-width:700px){#dongtian-overlay > .dt-runner{padding:0}.dt-question{padding:13px max(10px,env(safe-area-inset-right)) max(13px,env(safe-area-inset-bottom)) max(10px,env(safe-area-inset-left))}.dt-options{grid-template-columns:1fr;grid-auto-rows:minmax(52px,auto);overflow:visible}.dt-run-meta{grid-template-columns:repeat(3,1fr)}.dt-result-grid{grid-template-columns:repeat(3,1fr)}}
  `;
