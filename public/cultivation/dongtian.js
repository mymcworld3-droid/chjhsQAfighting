...
(function () {
  ...
  const css = `
    ...
    @media(max-width:560px){.dt-runner{padding-left:10px;padding-right:10px}.dt-question{padding:14px}.dt-run-meta{grid-template-columns:repeat(3,1fr)}.dt-result-grid{grid-template-columns:repeat(3,1fr)}}
  `;
  ...
  async function generateDongtian() {
    ...
    try {
      ...
      const response = await fetch('/api/generate-dongtian', {
        ...
        body: JSON.stringify({ text, images, creatorLevel: level })
      });
