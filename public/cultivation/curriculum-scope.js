...
parent.innerHTML='<div class="cs-scope" id="cs-scope"><p class="cs-note">選擇年級、科目、學期、版本，可跨章節勾選並複習。多數為108課綱整理單元；只有明確標示的出版社版本才有核對目次。</p><div class="cs-fields"><label>年級<select id="cs-grade"></select></label><label>科目<select id="cs-subject"></select></label><label>學期／冊次<select id="cs-term"></select></label><label>版本<select id="cs-edition"></select></label></div><label>搜尋章名或考點<input id="cs-search" placeholder="例如：方程式" maxlength="60"></label><div><button type="button" id="cs-all">勾選全部章節</button> <button type="button" id="cs-clear">取消勾選</button></div><div id="cs-units" class="cs-units"></div><button type="button" id="cs-add">加入勾選的單元</button><div class="cs-unit"><b>自訂複習範圍</b><input id="cs-custom" maxlength="90" placeholder="輸入任何科目的課程重點"><button type="button" id="cs-add-custom">加入自訂主題</button></div><p id="cs-message" role="status" class="cs-note"></p><p class="cs-note">國小一至六年級亦可選擇108課綱整理單元；出版社目錄節錄不代表完整課本。</p></div>';
