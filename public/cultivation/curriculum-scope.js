...
const subjectOptions=g=>g.startsWith('高中')?highSubjectList(g):Object.keys(files).filter(k=>k!=='資訊科技');
const sourcePath=(sub,g)=>g.startsWith('高中')?'high_school_unit_name/'+(highCore.has(sub)?'core.json':'humanities.json'):'middle_school_unit_name/'+files[sub];
...
parent.innerHTML='<div class="cs-scope" id="cs-scope"><p class="cs-note">選擇年級、科目、學期、版本，可跨章節勾選並複習。多數為108課綱整理單元；只有明確標示的出版社版本才有核對目次。</p><div class="cs-fields"><label>年級<select id="cs-grade"></select></label><label>科目<select id="cs-subject"></select></label><label>學期／冊次<select id="cs-term"></select></label><label>版本<select id="cs-edition"></select></label></div><label>搜尋章名或考點<input id="cs-search" placeholder="例如：方程式" maxlength="60"></label><div><button type="button" id="cs-all">勾選全部章節</button> <button type="button" id="cs-clear">取消勾選</button></div><div id="cs-units" class="cs-units"></div><button type="button" id="cs-add">加入勾選的單元</button><div class="cs-unit"><b>自訂複習範圍</b><input id="cs-custom" maxlength="90" placeholder="輸入任何科目的課程重點"><button type="button" id="cs-add-custom">加入自訂主題</button></div><p id="cs-message" role="status" class="cs-note"></p><p class="cs-note">國小保留自訂主題；高中可按學科與學年選擇108課綱整理單元。</p></div>';
...
async function render(){if(!init())return;const n=++serial;el('cs-grade').value=grade;subject=choices('cs-subject',subjectOptions(grade),subject,'選科目');units=[];el('cs-units').replaceChildren();
choices('cs-term',[],'','讀取中');choices('cs-edition',[],'','讀取中');
if(!grade.startsWith('國中')&&!grade.startsWith('高中')){display();status('這個年級目前請使用自訂複習範圍。');return;}
status('正在載入…');try{const data=await load();if(n!==serial)return;
let yearData;
if(grade.startsWith('高中')){
...
units=units.filter(u=>u.unit);display();if(units.length)status(grade.startsWith('高中') ? '108課綱複習單元：版本為整理版；已核對的出版社目次另行標示。請勾選章節或考點。' : '勾選整章或展開選取考點，再加入清單。');
...
function addChecked(){const groups=[...el('cs-units').querySelectorAll('.cs-chapter:checked')].map(c=>+c.dataset.i),chosen=new Set(groups),parts=[...el('cs-units').querySelectorAll('.cs-topic:checked')];const year=['七','八','九'][grades.indexOf(grade)-6];
const canonicalSubject=/^數學(?:A|B|甲|乙)$/.test(subject)?'數學':subject;
const path=grade.startsWith('高中')?[canonicalSubject,grade,term,subject,edition].join('/')
...
function addCustom(){const value=el('cs-custom').value.trim();if(!value)return status('請先輸入主題。');addList([{path:(/^數學(?:A|B|甲|乙)$/.test(subject)?'數學':subject)+'/'+grade+'/自訂/'+subject,detail:value,sub_topics:[]}]);el('cs-custom').value='';}
...
window.renderSoloUnitSelectors=async()=>{const x=el('set-level')?.value;if(!el('cs-scope')){if(grades.includes(x))grade=x;else if(x==='高中職')grade='高中一年級';}await render();};
