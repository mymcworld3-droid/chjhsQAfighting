...
function addChecked(){const groups=[...el('cs-units').querySelectorAll('.cs-chapter:checked')].map(c=>+c.dataset.i),chosen=new Set(groups),parts=[...el('cs-units').querySelectorAll('.cs-topic:checked')];const year=['七','八','九'][grades.indexOf(grade)-6],path=[subject,year+(term==='第一學期'?'上':term==='第二學期'?'下':'全學年'),edition].join('/');
