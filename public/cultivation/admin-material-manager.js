...
(function () {
  ...
  function render() {
    ...
    if (materialList) {
      materialList.innerHTML = MATERIAL_CATALOG.map((item) => `<article class="amm-item"><div class="amm-icon">${escapeHtml(item.icon || '材')}</div><div class="amm-copy"><strong>${escapeHtml(item.name)}</strong><div class="amm-meta">${escapeHtml(item.id)} · ${escapeHtml(item.category)}${item.weaponForm ? ' · 器型 ' + escapeHtml(item.weaponForm) : ''} · 坊市參考 ${materialMarketReferencePrice(item.realm).toLocaleString()} 金幣 · ${item.buyGold > 0 ? `系統採購 ${item.buyGold} 金幣` : '不可直接採購'}</div><div class="amm-description">${escapeHtml(item.description || '')}</div></div><div class="amm-actions"><button type="button" class="amm-edit" data-material-edit="${escapeHtml(item.id)}"><i class="fa-solid fa-pen"></i> 編輯</button><button type="button" class="amm-delete" data-material-delete="${escapeHtml(item.id)}"><i class="fa-solid fa-trash"></i> 刪除</button></div></article>`).join('');
    }
  }
  ...
  function openMaterialEditor(item = null) {
    ...
    modal.innerHTML = `<section class="amm-card" role="dialog" aria-modal="true"><h3>${editing ? '編輯材料' : '新增材料'}</h3><p class="amm-note">材料 ID 建立後會鎖定。採購價設為 0 代表玩家不能直接用金幣取得，可留給洞天、鬥法或活動掉落。坊市參考價依材料境界統一計算，與這裡的採購價分開。</p><div class="amm-grid"><div class="amm-field"><label>材料 ID（英文小寫與 -）</label><input id="amm-id" maxlength="64" ${editing ? 'readonly' : ''} value="${escapeHtml(item?.id || '')}" placeholder="例如 star-sand"></div><div class="amm-field"><label>名稱</label><input id="amm-name" maxlength="80" value="${escapeHtml(item?.name || '')}"></div><div class="amm-field"><label>圖示（1–4 字）</label><input id="amm-icon" maxlength="4" value="${escapeHtml(item?.icon || '材')}"></div><div class="amm-field"><label>分類</label><select id="amm-category">${MATERIAL_CATEGORIES.map((category) => `<option value="${escapeHtml(category)}" ${category === (item?.category || '其他') ? 'selected' : ''}>${escapeHtml(category)}</option>`).join('')}</select></div><div class="amm-field"><label>正式器型（選填；煉器優先保留）</label><select id="amm-weapon-form">${MATERIAL_WEAPON_FORMS.map((form) => `<option value="${escapeHtml(form)}" ${form === (item?.weaponForm || '') ? 'selected' : ''}>${escapeHtml(form || '不指定')}</option>`).join('')}</select></div><div class="amm-field"><label>指定兵器名稱（選填）</label><input id="amm-weapon-name" maxlength="80" value="${escapeHtml(item?.weaponName || '')}" placeholder="例如 玄鐵劍"></div><div class="amm-field"><label>採購價（金幣；0 = 不可購買）</label><input id="amm-buy-gold" type="number" min="0" step="1" value="${Math.max(0, Number(item?.buyGold) || 0)}"></div><div class="amm-field full"><label>說明／材質特性</label><textarea id="amm-description" maxlength="500">${escapeHtml(item?.description || '')}</textarea></div><div class="amm-field full"><label>材料背景故事（煉器 AI 會參考）</label><textarea id="amm-story" maxlength="2000" placeholder="材料來源、傳說、過往用途、特殊歷史……">${escapeHtml(item?.story || '')}</textarea></div></div><div id="amm-status" class="amm-status"></div><div class="amm-modal-actions"><button type="button" class="amm-cancel">取消</button><button type="button" class="amm-save">${editing ? '儲存變更' : '建立材料'}</button></div></section>`;
  ...
  async function saveMaterial(modal, originalId) {
    ...
    const raw = {
      ...
      realm: MATERIAL_CATALOG.find((entry) => entry.id === id)?.realm || '煉氣',
      weaponForm: modal.querySelector('#amm-weapon-form').value,
