...
(function () {
  ...
  function score() {
    ...
    return Number(data?.stats?.score ?? data?.score ?? 0) || 0;
  }
  ...
  function injectStyle() {
    ...
    style.textContent = `
      :root { --xq-gold:#e9c46a; --xq-cyan:#72d6d0; --xq-purple:#9b8cff; }
      ...
      .xiuxian-kicker { color:var(--xq-gold); font-size:10px; letter-spacing:.28em; font-weight:900; text-transform:uppercase; }
      ...
      .xiuxian-hidden { display:none !important; }
    `;
  ...
  function addHomePanel() {
    ...
    document.getElementById('xiuxian-path').addEventListener('click', () => {
      const lines = REALMS.map(r => `${r.emoji} ${r.name}：${r.need} 修為起`).join('\n');
      alert(lines);
    });
  }
  ...
  function rewriteLabels() {
    ...
    text('[data-i18n="app_name"]', '修仙問道');
    text('[data-i18n="app_title"]', '修仙世界 · 問道試煉');
    text('[data-i18n="welcome_title"]', '踏入仙途');
    text('[data-i18n="welcome_desc"]', '答題悟道 × 真人鬥法 × 境界突破');
    ...
    text('[data-i18n="history_title"]', '悟道紀錄');
    text('[data-i18n="rank_title"]', '九州仙榜');
    text('[data-i18n="th_rank"]', '境界');
    text('[data-i18n="stat_score"]', '修為');
    text('[data-i18n="stat_streak"]', '連勝道心');
    text('[data-i18n="stat_best_streak"]', '最高連勝');
  ...
  function render() {
    ...
    if (rank) rank.textContent = `${realm.emoji} ${realm.name}`;
    ...
    if (label) label.textContent = next ? `下一境界：${next.name}` : '已登仙，繼續悟道';
