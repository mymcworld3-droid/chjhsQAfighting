...
// 修煉 v3：金丹頁只保留「金丹、名稱/品質、洗髓、機率說明、裝配」。
(function () {
  'use strict';

  const GOLDEN_CORE_SCORE = 150;
  const WASH_COST = 100;
  const STATE_KEY = 'xiuxian_training_state_v3';
  const LEGACY_KEYS = ['xiuxian_training_state_v2', 'xiuxian_training_state_v1'];
  ...
  const CORE_TYPES = [
    {
      ...
      resolve({ isCorrect, grade }) {
        ...
        return randomPercent(chance) ? { bonusGain: 1, message: `${this.name}潮聲大作，本次修為翻倍` } : {};
      }
    },
    {
      ...
      resolve({ isCorrect, grade, counters }) {
        ...
        return counters.correct % interval === 0 ? { bonusGain: 1, message: `${this.name}回元，額外 +1 修為` } : {};
      }
    },
    {
      ...
      resolve({ isCorrect, grade, previousStreak }) {
        ...
        return previousStreak >= threshold ? { forceShield: true, message: `${this.name}凝神，道心護體成形` } : {};
      }
    },
    {
      ...
      resolve({ isCorrect, grade, score }) {
        ...
        return next && next - score <= range ? { bonusGain: 1, message: `${this.name}助你破境，額外 +1 修為` } : {};
      }
    },
    {
      ...
      resolve({ isCorrect, grade, previousStreak }) {
        return isCorrect && previousStreak >= Math.max(1, grade) ? { bonusGain: 1, message: `${this.name}引星入體，額外 +1 修為` } : {};
      }
    },
    {
      ...
      resolve({ isCorrect, grade, hasShield }) {
        ...
        return randomPercent(chance) ? { preserveShield: true, message: `${this.name}護住道心，護體未散` } : {};
      }
    },
    {
      ...
      resolve({ isCorrect, grade }) {
        ...
        return randomPercent(chance) ? { forceShield: true, message: `${this.name}雷光護體` } : {};
      }
    },
    {
      ...
      resolve({ isCorrect, grade, previousStreak }) {
        ...
        return isCorrect && previousStreak + 1 === threshold ? { bonusGain: 2, message: `${this.name}倒轉氣機，額外 +2 修為` } : {};
      }
    }
  ];
  ...
  function uid() {
    return `core_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  }

  function normalizeCore(core, fallback = null) {
    if (!core || typeof core !== 'object') return fallback;
    return {
      instanceId: core.instanceId || uid(),
  ...
  function starterCore() {
    return { instanceId: 'starter_core', type: 'taichu', grade: 9, createdAt: Date.now() };
  }
  ...
  function migrate(raw) {
    ...
    // v2/v1 used state.core as the equipped golden core.
    const legacyCore = normalizeCore(raw.equipped || raw.core, base.equipped);
    const preview = normalizeCore(raw.preview, legacyCore);
    const inv = Array.isArray(raw.inventory)
      ? raw.inventory.map((item) => normalizeCore(item)).filter(Boolean)
      : [];

    if (!inv.some((item) => item.instanceId === legacyCore.instanceId)) inv.unshift({ ...legacyCore });
  ...
  function toast(message) {
    document.getElementById('training-v3-toast')?.remove();
    const el = document.createElement('div');
    el.id = 'training-v3-toast';
  ...
  function randomCore() {
    ...
    return { instanceId: uid(), type: type.id, grade: randomGrade(), createdAt: Date.now() };
  }
  ...
  function coreTabMarkup() {
    const core = state.preview || state.equipped;
    const type = coreType(core.type);
    const equipped = state.equipped?.instanceId === core.instanceId;
    return `
      <section class="core-minimal-card">
        <div class="core-minimal-center">
          ...
          <button id="equip-preview-core" type="button" class="core-equip-btn ${equipped ? 'equipped' : ''}" ${equipped || busy ? 'disabled' : ''}>
            ${equipped ? '<i class="fa-solid fa-circle-check"></i> 已裝配此金丹' : '<i class="fa-solid fa-circle-dot"></i> 裝配此金丹'}
          </button>
        </div>
      </section>
    `;
  ...
  function bagTabMarkup() {
    if (!state.inventory.length) {
      return `<section class="training-v3-empty"><i class="fa-solid fa-box-open"></i><h3>背包尚空</h3><p>洗髓得到的金丹會保存在這裡。</p></section>`;
    }

    return `
      <section class="training-v3-bag-grid">
        ${state.inventory.slice().reverse().map((core) => {
          const type = coreType(core.type);
          const equipped = state.equipped?.instanceId === core.instanceId;
          return `
            <button type="button" class="training-v3-bag-item ${equipped ? 'equipped' : ''}" data-bag-core="${core.instanceId}">
              <span class="training-v3-mini-core core-tone-${type.tone}">${type.icon}</span>
              <span class="training-v3-bag-copy"><strong>${type.name}</strong><small>${core.grade} 品${equipped ? ' · 本命' : ''}</small></span>
            </button>
          `;
        }).join('')}
      </section>
  ...
  function createTrainingPage() {
    ...
    page.querySelectorAll('[data-training-tab]').forEach((button) => {
      button.addEventListener('click', () => {
        ...
        page.querySelectorAll('[data-training-tab]').forEach((tab) => tab.classList.toggle('active', tab.dataset.trainingTab === activeTab));
        renderTrainingPage();
      });
    });
    renderTrainingPage();
  ...
  function bindCoreActions() {
    document.getElementById('training-core-orb')?.addEventListener('click', () => showCoreDetails(state.preview || state.equipped));
    ...
    document.getElementById('equip-preview-core')?.addEventListener('click', equipPreview);
  }

  function bindBagActions() {
    document.querySelectorAll('[data-bag-core]').forEach((button) => {
      button.addEventListener('click', () => {
        const found = state.inventory.find((core) => core.instanceId === button.dataset.bagCore);
        if (!found) return;
        state.preview = { ...found };
        saveLocal();
        activeTab = 'core';
        const page = document.getElementById('page-training');
        page?.querySelectorAll('[data-training-tab]').forEach((tab) => tab.classList.toggle('active', tab.dataset.trainingTab === 'core'));
        renderTrainingPage();
      });
    });
  }
  ...
  async function washCore() {
    ...
    busy = true;
    renderTrainingPage();
    const previousPreview = state.preview ? { ...state.preview } : null;
    const previousInventory = state.inventory.map((item) => ({ ...item }));
  ...
  async function equipPreview() {
    ...
    if (state.equipped?.instanceId === state.preview.instanceId) return;
    const old = state.equipped ? { ...state.equipped } : null;
    busy = true;
    state.equipped = { ...state.preview };
    renderTrainingPage();
    try {
      await persistRemote();
      toast(`已裝配：${state.equipped.grade} 品 ${coreType(state.equipped.type).name}`);
    } catch (error) {
  ...
  // 只有「已裝配」金丹的效果會進入答題結算；洗髓後未裝配的預覽金丹不生效。
  window.resolveGoldenCoreCultivationReward = function ({ stats, isCorrect }) {
    if (!isUnlocked() || !state.equipped) return { bonusGain: 0, message: '' };
    const type = coreType(state.equipped.type);
    const grade = clampGrade(state.equipped.grade);
